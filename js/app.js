...
function renderCounting() {
  // V3.3.62: Algemeen blijft altijd zichtbaar in Voorraad en heeft geen gebruiksstatus.
  ...
  const ps = allProductsOrdered().filter(p =>
    p.externalProduct !== true &&
    (p.mode === "general" || isInUse(p) || stockUnits(p) > 0)
  );
  const groups = [];
  ps.forEach(p => {
  ...
  countList.innerHTML = groups.length ? groups.map(g => {
    ...
    return `<div class="item count-card product-family-card">
      <div class="item-head"><div><strong>${esc(g.name)}</strong>${g.mode === "general" ? "" : `<div class="count-meta">${inUse ? `<span class="badge use-yes">In gebruik</span>` : `<span class="badge use-no">Niet in gebruik</span>`}</div>`}</div><div class="family-total">Totaal<br><strong>${fmt(total)} ${esc(looseUnitLabel(family[0], total) || unit)}</strong></div></div>
...
// V3.3.62 - digitaal afbeeldingsbestand of camera; robuustere tabelherkenning en samenvoegen.
...
function overviewAttentionItems() {
  ...
  sondeNames.forEach(name => {
    ...
    const p = products.find(activeProduct) || products[0];
    if (!p) return;
    // V3.3.62: gebruik exact hetzelfde familie-besteladvies als op Bestellen.
    ...
    const orderUnits = products
      ...
      .reduce((sum, x) => sum + Number(adviceForProduct(x).orderUnits || 0), 0);
    ...
    if (orderUnits > 0 || orderedPackages > 0 || thtSummary.attention) {
      items.push({p, name, orderUnits, orderedPackages, stock, days:familyDaysSupply(p), unused:false, tht:thtSummary.html, mode:"sonde"});
    }
    products.filter(x => stockUnits(x) > 0 && !isInUse(x)).forEach(x => {
...
async function createSchedulePdf(unit,dateValue){
  ...
  let y=top;
  doc.setTextColor(45,45,55);doc.setFont("helvetica","bold");doc.setFontSize(11.5);
  doc.text(`Bijvoeding aftekenlijst - Unit ${unit} | Week ${week} | ${longDate(dates[0])} t/m ${longDate(dates[6])}`,ml,y+4);y+=8;
  const drawHeader=()=>{
    ...
    dates.forEach((d,i)=>{
      ...
      const cx=ml+roomW+amountW+detailW+i*dayW+dayW/2,dayTop=y+headerH/2-1.3;
      doc.setFont("helvetica","bold");doc.setFontSize(fs+0.2);doc.text(dayNames[i],cx,dayTop,{align:"center"});
      doc.setFont("helvetica","bold");doc.setFontSize(fs);doc.text(fmtDate(d),cx,dayTop+3.3,{align:"center"});
    }); y+=headerH;
  };
  drawHeader();
  groups.forEach((g,gi)=>{
...
async function mergeUnitWeekPair(unit, scheduleFile, weeklyFile, weekDate){
  if(!window.PDFLib?.PDFDocument){
    alert("De PDF-bundelmodule kon niet worden geladen. Controleer de internetverbinding en probeer opnieuw.");
    return null;
  }
  const merged=await PDFLib.PDFDocument.create();
  for(const file of [scheduleFile,weeklyFile]){
    if(!file) continue;
    const source=await PDFLib.PDFDocument.load(await file.arrayBuffer());
    const pages=await merged.copyPages(source,source.getPageIndices());
    pages.forEach(page=>merged.addPage(page));
  ...
  const bytes=await merged.save();
  const week=weekInfo(weekDate).week;
  return new File([bytes],`Bijvoeding-Unit-${unit}-week-${week}.pdf`,{type:"application/pdf"});
}
...
async function createWeeklyQuantitiesPdf(unit,dateValue){
...
async function makeSelectedSchedules(){
  ...
  // Als aftekenlijst + weekhoeveelheden samen zijn gekozen, maak per unit en week
  // één bestand: pagina 1 aftekenlijst, pagina 2 weekhoeveelheden.
  if(types.includes("check") && types.includes("weekly")){
    for(const unit of units){
      for(const weekDate of weeks){
        const schedule=await createSchedulePdf(unit,weekDate);
        const weekly=await createWeeklyQuantitiesPdf(unit,weekDate);
        if(schedule && weekly){
          const bundled=await mergeUnitWeekPair(unit,schedule,weekly,weekDate);
    ...
    if(types.includes("weekly")){
      const weeklyDate=weeks[0]||new Date().toISOString().slice(0,10);
      for(const unit of units){const f=await createWeeklyQuantitiesPdf(unit,weeklyDate);if(f)files.push(f);}
    }
...
function stockMailAvailableUnits(p) {
  return Math.max(0, stockUnits(p));
}

function stockMailUnitLabel(p, n) {
  if (hasLooseUnits(p)) return looseUnitLabel(p, n);
  return plural(p.orderUnit || p.consumptionUnit || "stuks", n);
}

function stockMailQuantityText(p, amount = stockMailAvailableUnits(p)) {
  return `${fmt(amount)} ${stockMailUnitLabel(p, amount)}`;
}

function toggleStockMailProduct(id, checked) {
  const qty = document.querySelector(`.stock-mail-qty[data-product-id="${CSS.escape(String(id))}"]`);
  if (!qty) return;
  qty.disabled = !checked;
  if (checked && !(Number(qty.value) > 0)) qty.value = String(qty.max || 1);
}
...
function renderStockMailFamilyList() {
  ...
  products.forEach(p => {
    ...
    const e = expiryInfo(p);
    const status = e.expired ? "Houdbaarheid verlopen" : e.soon ? "Kort houdbaar" : (p.expiryDate ? "" : "Geen houdbaarheid ingevuld");
    const max = stockMailAvailableUnits(p);
    html.push(`<div class="stock-mail-family stock-mail-product-row">
      <input type="checkbox" class="stock-mail-family-check" value="${esc(p.id)}" onchange="toggleStockMailProduct('${esc(p.id)}', this.checked)">
      <span class="stock-mail-product-info">
        <strong>${esc(stockMailDisplayName(p))}</strong>
        <small>Voorraad: ${esc(stockMailQuantityText(p))} · Houdbaar tot ${esc(stockMailThtText(p))}${status ? ` · ${esc(status)}` : ""}</small>
      </span>
      <label class="stock-mail-qty-wrap">
        <span>Aantal</span>
        <input type="number" class="stock-mail-qty" data-product-id="${esc(p.id)}" min="1" max="${esc(max)}" step="1" value="${esc(max)}" disabled>
      </label>
    </div>`);
  });
...
function setAllStockMailFamilies(checked) {
  document.querySelectorAll(".stock-mail-family-check").forEach(input => {
    input.checked = checked;
    toggleStockMailProduct(input.value, checked);
  });
}
...
function stockMailThtText(p) {
  return p.expiryDate ? formatExpiryMonth(p.expiryDate) : "niet ingevuld";
}

function createStockMail() {
  const checked = [...document.querySelectorAll(".stock-mail-family-check:checked")];
  if (!checked.length) {
  ...
  for (const input of checked) {
    const p = data.products.find(x => String(x.id) === String(input.value));
    if (!p) continue;
    const qtyInput = document.querySelector(`.stock-mail-qty[data-product-id="${CSS.escape(String(p.id))}"]`);
    const max = stockMailAvailableUnits(p);
    let amount = Number(qtyInput?.value || 0);
    if (!Number.isFinite(amount) || amount < 1) amount = 1;
    amount = Math.min(Math.floor(amount), Math.floor(max));
    if (amount > 0) selected.set(String(p.id), amount);
  }

  const products = stockMailProducts().filter(p => selected.has(String(p.id)));
  if (!products.length) {
      ...
      .forEach(([family, familyRows]) => {
        ...
        familyRows
          ...
          .forEach(p => {
            ...
            const amount = selected.get(String(p.id));
            body.push(`  ${showVariant ? `- ${variant}: ` : "- "}${stockMailQuantityText(p, amount)} | Houdbaar tot ${stockMailThtText(p)}${e.expired ? " | VERLOPEN" : ""}`);
          });
        body.push("");
      });
  });
